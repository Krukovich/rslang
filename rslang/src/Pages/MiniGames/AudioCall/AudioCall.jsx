...
export class AudioCall extends React.Component {
  ...
  randomWords = async ( page, group ) => {
    ...
    writeStats (userIdState, stateStats) {
      ...
      const token = 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpZCI6IjVlZjEyZDdiYWEyNDVlMDAxN2E1NzkyZiIsImlhdCI6MTU5Mjg4NjM5OCwiZXhwIjoxNTkyOTAwNzk4fQ.XW6eOGXB8D9YJioyDi9L2DZNPwekkHhnJwksHNq1TkU';
...
createUserWord(userIdState, stateStats)
  ...
  render() {
      return (
        <AlertRed showAlert={this.state.showAlert} HeadText={'Вы прошли уровень'} MainText={'верно ответили на ' + this.state.gameSuccessAnswer}>
       <div className="AudioCall vh-100">  
           <h1>AudioCall</h1>
           
           <div className="text-center">
            <ButtonStartGame startGame={() => this.startGame()} gameStart={this.state.gameStart} gameStage={this.state.gameStage}/>

           </div>           
           <AudioComp gameStart={this.state.gameStart} gameFindWord = {this.state.gameFindWord} gameStage={this.state.gameStage}/>
           <Words 
            ...
            />   
            <div className="text-center">
            <ButtonNextStage className="align-center" nextStage={() => this.nextStage()} gameStart={this.state.gameStart} gameButton={this.state.gameButton}/>
            </div>
...
function ButtonStartGame (props) {
  if(props.gameStart) {
    return (
      <div><span>игра начата</span> уровень №{props.gameStage}</div>
    )
  } else {
    return (
      <button className="btn btn-primary " onClick={() => props.startGame()}>Начать игру</button>
    )
  }  
}
...
function ButtonNextStage (props) {
  if(props.gameStart) {
    return (
      <button className="btn btn-primary " onClick={() => props.nextStage()}>{props.gameButton}</button>
    )
  } else {
    return (
      null
